...
class XPUTestAbsOPComplex(XPUOpTestWrapper):
    ...
    class XPUTestAbsComplex(TestActivationOPBase):
        ...
        def gen_random_part(self, rng):
            if self.dist == "normal":
                val = rng.normal(
                    loc=self.mean, scale=self.std, size=self.shape
                )
            else:
                val = rng.uniform(
        ...
        def set_case(self):
            ...
            real_part = np.where(np.abs(real_part) < 0.005, np.float32(0.02), real_part)
            imag_part = np.where(np.abs(imag_part) < 0.005, np.float32(0.02), imag_part)
